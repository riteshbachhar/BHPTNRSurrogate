...
[![Unit Tests](https://github.com/riteshbachhar/BHPTNRSurrogate/actions/workflows/tests.yml/badge.svg)](https://github.com/riteshbachhar/BHPTNRSurrogate/actions/workflows/tests.yml)
...
```bash
git clone https://github.com/riteshbachhar/BHPTNRSurrogate.git
cd BHPTNRSurrogate
pip install -e .
```
...
The surrogate data files (~1.7GB total) must be downloaded separately and placed in the `data/` directory:
...
Unit tests (`test_check_inputs.py`, `test_utils.py`) run without data files. Integration tests (`test_generate_waveforms.py`) require the HDF5 data files and are automatically skipped if they are not present.

## Examples

Example tutorial notebooks for the **BHPTNRSur1dq1e4** and **BHPTNRSur2dq1e3** models are available in `tutorials/`.

```bash
jupyter notebook tutorials/BHPTNRSur1dq1e4.ipynb
```
...
https://github.com/riteshbachhar/BHPTNRSurrogate/issues
